...
def teleport_loop(current_state, pose_client, airsim_client, potential_states_fetcher, file_manager, loop_mode, parameters):
    ...
    initialize_empty_frames(airsim_client.linecount, pose_client, current_state, plot_loc, photo_loc)
    airsim_client.linecount += 1
    ...
    while airsim_client.linecount < airsim_client.length_of_simulation:    
        ...
        if pose_client_sim.find_best_traj: #/and exp_ind >= predefined_traj_len:
            for state_ind in range(0, len(potential_states_fetcher.potential_states_try)):
                ...
                for trial_ind in range(pose_client_sim.num_of_noise_trials):
                    plot_loc, photo_loc = file_manager.plot_loc, file_manager.get_photo_loc()
                    proc_pool.apply_async(run_optimization, args=(pose_client, airsim_client.linecount, current_state, plot_loc, photo_loc, trial_ind, ))

                while True:
                    with num_finished.get_lock():
                        #print("yet another something")
                        if num_finished.value >= pose_client_sim.num_of_noise_trials:
                            assert (num_finished.value == pose_client_sim.num_of_noise_trials)
                            break
                    time.sleep(1)

                with num_finished.get_lock():
                    num_finished.value = 0
                assert(que.qsize() == pose_client_sim.num_of_noise_trials)

                for _ in range(pose_client_sim.num_of_noise_trials):
                    trial_ind, pose_client_copy = que.get()
        ...
        plot_loc, photo_loc = file_manager.plot_loc, file_manager.get_photo_loc()
        determine_positions(airsim_client.linecount, pose_client, current_state, plot_loc, photo_loc)
...
def run_optimization(pose_client, linecount, current_state, plot_loc, photo_loc, trial_ind):
    #print("hi!")
    pose_client_copy = pose_client.deepcopy_PEC(trial_ind)
    #print("i am here!")
    determine_positions(linecount, pose_client_copy, current_state, plot_loc, photo_loc)
    #print("i am here2!")

    que.put((trial_ind, pose_client_copy))
    #print("i am here3!")
    with num_finished.get_lock():
