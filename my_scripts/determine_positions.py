...
def determine_3d_positions_energy_scipy(airsim_client, pose_client, measurement_cov_, plot_loc = 0, photo_loc = 0, quiet=True):
    ...
    bone_2d, heatmap_2d, _, _ = determine_2d_positions(pose_client.modes["mode_2d"], pose_client.cropping_tool, True, unreal_positions, bone_pos_3d_GT, cropped_image, scales)
    if (quiet == False):
    ...
    if (airsim_client.linecount != 0):
        ...
        pose_client.kalman.init_state(np.reshape(a = pre_pose_3d, newshape = [3*num_of_joints,1], order = "C"))
    ...
    if (airsim_client.linecount >0):
        ...
        #P_world_scrambled, _ = leastsq(objective.forward, pose3d_init, Dfun=objective_jacobian, ftol=pose_client.ftol)
        ...
        #kalman update
        pose_client.kalman.update(z=np.reshape(a = pre_kalman_pose, newshape = [3*num_of_joints,1], order = "C"), noise_cov=cov)
        #use kalman results as pose 
        kalman_res = np.reshape(a = pose_client.kalman.x_post, newshape =[3, num_of_joints], order = "C")
...
def determine_3d_positions_energy_pytorch(airsim_client, pose_client, measurement_cov_, plot_loc = 0, photo_loc = 0):
    unreal_positions, bone_pos_3d_GT, drone_pos_vec, angle = airsim_client.getSynchronizedData()
    bone_connections, joint_names, num_of_joints, bone_pos_3d_GT = model_settings(pose_client.model, bone_pos_3d_GT)
    input_image = cv.imread(photo_loc)
    cropped_image = pose_client.cropping_tool.crop(input_image)
...
def determine_3d_positions_backprojection(airsim_client, pose_client, measurement_cov_, plot_loc = 0, photo_loc = 0):
    unreal_positions, bone_pos_3d_GT, drone_pos_vec, angle = airsim_client.getSynchronizedData()
    bone_connections, joint_names, _, bone_pos_3d_GT = model_settings(pose_client.model, bone_pos_3d_GT)

    bone_2d, _, _, _ = determine_2d_positions(pose_client.modes["mode_2d"], pose_client.cropping_tool, False, unreal_positions, bone_pos_3d_GT, photo_loc, -1)
...
def determine_3d_positions_all_GT(airsim_client, pose_client, plot_loc, photo_loc):
    ...
    if (modes["mode_2d"] == 1):
        ...
        bone_2d, heatmap_2d, _, _ = determine_2d_positions(pose_client.modes["mode_2d"], pose_client.cropping_tool, False, unreal_positions, bone_pos_3d_GT, photo_loc, scale_)
        superimpose_on_image(bone_2d, plot_loc, airsim_client.linecount, bone_connections, photo_loc, custom_name="gt_", scale = scale_)
